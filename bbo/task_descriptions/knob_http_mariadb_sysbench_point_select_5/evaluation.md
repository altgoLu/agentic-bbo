...
- **Metrics:** primary = `throughput` == parsed **per-second** figure from the sysbench text report for **``oltp_point_select``**; if `transactions` is missing for a script, the server falls back to a `queries` line when present.
- **Request contract:** the Python `HttpDatabaseKnobTask` sends:
  - `workload: "point_select"` (server maps to **``oltp_point_select``**)
