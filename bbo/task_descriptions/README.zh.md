...
- **离线 / HTTP surrogate（canonical `knob_surrogate_*`）：**如 `knob_surrogate_sysbench_5/`、`knob_surrogate_job_5/` 等；说明 sklearn 代理设定。数据与实现已并入 `bbo/tasks/dbtune/`（`assets/`、进程内任务、可选 `docker_surrogate/`）。文内路径应写 `bbo/tasks/dbtune/...`（原 `database/`、`surrogate/` 已移除合并）。
