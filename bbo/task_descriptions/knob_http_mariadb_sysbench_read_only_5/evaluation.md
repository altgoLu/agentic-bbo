...
- **Metrics:** primary = `throughput` == parsed **per-second** figure from the sysbench text report for **``oltp_read_only``**; if `transactions` is missing for a script, the server falls back to a `queries` line when present.
- **Request contract:** the Python `HttpDatabaseKnobTask` sends:
  - `workload: "read_only"` (server maps to **``oltp_read_only``**)
